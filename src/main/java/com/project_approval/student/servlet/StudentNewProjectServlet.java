...
public class StudentNewProjectServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException {
		...
		try {
			...
			switch (status) {
			case (0): {
				session.setAttribute("newProjectMessage", "Fail to Add New Project");
				break;
			}
			case (1): {
				session.setAttribute("newProjectMessage", "File Upploaded But Issue In Storing project details");
				break;
			}
			case (2): {
				session.setAttribute("newProjectMessage", "New Project Added Sucessfully");
				break;
			}
			}
